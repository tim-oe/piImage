...
alias flushnet='sudo ip -s -s neigh flush all && sudo ip route flush cache && { systemctl is-active --quiet avahi-daemon && sudo systemctl restart avahi-daemon; true; } && sudo systemctl restart NetworkManager'
...
alias iwconfig='iw dev wlan0 link && iw dev wlan0 info'
...
alias espterm='poetry run rshell -p /dev/ttyACM0 repl'